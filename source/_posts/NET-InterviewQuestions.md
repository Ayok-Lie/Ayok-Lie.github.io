...
- .Net面试题
title: 面试题库记载—1
updated: '2025-10-15T15:51:11.616+08:00'
---
#### 异步开发实现有哪些方式
...
async/await是编译器实现的语法糖，底层基于任务（Task）和状态机（State Machine）。当遇到await时，方法会将后续操作包装成一个回调，然后立即返回，释放当前线程去处理其它请求。当异步操作完成时，回调会被调度执行，恢复方法的执行。

在Web API中，I/O操作（如数据库查询，文件读写，HTTP调用）时主要的性能瓶颈。如果使用同步方法，线程会被阻塞，无法处理其他请求，导致服务器资源浪费和吞吐量下降。使用async/await可以让线程在等待I/O完成时去处理其它请求，极大地提高服务器的并发处理能力和资源理由率

如果一个async方法内部没有await，编译器会发出警告，该方法会以同步的方式执行，失去了异步的意义。

#### Task.WhenAll vs Parallel.ForEach

Task.WhenAll：用于并发执行多个Task。它等待所以任务完成。适用与I/O密集型操作，如同事发起多个HTTP请求，数据库查询。它不创建新线程，利用I/O完成端口。

Parallel.ForEach：用于并行处理数据集合。它使用线程池中的多线程来同时处理集合中的元素。适用于CPU密集型操作，如对大量数据进行处理，图像处理
